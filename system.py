...
def object_detection(im_filename, flag, prev):
    os.chdir("../darknet/build/darknet/x64/")
    if flag:
        yolo = os.popen(
        "powershell.exe ./darknet.exe detector test data/obj.data yolo-obj-test-newanchor.cfg backup/yolo-obj-newanchor_23972_0.917123.weights -dont_show -thresh 0.5 " + im_filename)
    else:
        yolo = os.popen(
            "powershell.exe ./darknet.exe detector test data/obj2.data yolo-obj2-test.cfg backup/yolo-obj2_29169_0.173446.weights -dont_show -thresh 0.4 " + im_filename)
    output = yolo.read()
    yolo.close()
    if len(output.split("\n")) == 4:
        if flag:
        ...
        write_log(str(person_index) + ".txt", prev)
        ...
        for each in keypoints:
            ...
            if count == 0:
                ...
                for j in range(len(ref_points)):
                    if la.norm(each[name["Neck"]] - ref_points[j]) < 10:
                        ref_points[j] = each[name["Neck"]]
                        person_index = j
                        break
            norm_each = skeleton_norm(each)
            bone_img = draw_skeleton(norm_each)
            io.imsave("./bone_image/" + str(person_index) + "_" + str(count) + ".jpg", bone_img)
            label = evaluate("./bone_image/" + str(person_index) + "_" + str(count) + ".jpg")
            # action: take the note, use the cellphone, and use the laptop
            if label == 0:
                object_img = crop_object(img, each, True)
                filename = os.path.join("E:/Desktop/system/crop1/", str(person_index)) + "_" + str(count) + ".jpg"
                object_label = object_classification(object_img[..., ::-1], "3class_trainall.h5")
                if object_label == 0:
                    cv2.imwrite(filename, object_img)
                    obj_label = object_detection(filename, True, "0")
                    write_log(str(person_index) + ".txt", obj_label)
                elif object_label == 1:
                    write_log(str(person_index) + ".txt", "2")
                elif object_label == 2:
                    write_log(str(person_index) + ".txt", "1")
                else:
                    write_log(str(person_index) + ".txt", "4")
            # action: raise a hand
            elif label == 1:
                write_log(str(person_index) + ".txt", "6")
            # action: tilt the head
            elif label == 2:
                write_log(str(person_index) + ".txt", "7")
            # action: support the head, pick the phone
            elif label == 3:
                object_img = crop_object(img, each, False)
                filename = os.path.join("E:/Desktop/system/crop2/", str(person_index)) + "_" + str(count) + ".jpg"
                object_label = object_classification(object_img[..., ::-1], "2class_trainall.h5")
                if object_label == 1:
                    cv2.imwrite(filename, object_img)
                    obj_label = object_detection(filename, False, "8")
                    write_log(str(person_index) + ".txt", obj_label)
                elif object_label == 0:
